/**
 * API- http://localhost:3000/api/chat/group/groupremove
 * this api is used to remove a user from a group chat through put request..
 * Request body contains user id(mongoDB user object _id) and target the group chat id(mongoDB chat object _id).
 * request body example:
 *  {
 *    removeUserID: '88879bjhf00230323',
...
async function handler(req, res) {
    ...
    //checking user authentication to access this api route...
      ...
      const {removeUserID,chatID}=JSON.parse(req.body)

      if(!removeUserID || !chatID){
        return res.status(400).json({error:'chat group id or user id is missing...'})
      }


     const result=await Chat.findByIdAndUpdate({_id:chatID},{$pull:{'users':removeUserID}},{new:true})
        .populate('users','-password')
        .populate('groupAdmin','-password')

      if(!result){
       return res.status(400).json({error:'remove user to group chat failed...'})
      }   

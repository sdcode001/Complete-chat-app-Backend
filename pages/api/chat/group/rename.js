/**
 * API- http://localhost:3000/api/chat/group/rename
 * this api is used to rename a group chat through put request..
 * Request body contains new group name and the group chat id(mongoDB chat object _id).
...
async function handler(req, res) {
    ...
    //checking user authentication to access this api route...
      ...
      const {newName,chatID}=JSON.parse(req.body)
      ...
      const result=await Chat.findByIdAndUpdate({_id:chatID},{chatName:newName},{new:true})
         .populate('users','-password')
         .populate('groupAdmin','-password')

      if(!result){
        return res.status(400).json({error:'group name update failed...'})
      }   
